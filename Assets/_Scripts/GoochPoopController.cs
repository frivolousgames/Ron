...
public class GoochPoopController : MonoBehaviour
{
    Vector3 target;
    [SerializeField]
    Transform spawnPos;
    ...
    {
        target = GoochController.selectedTargetPos;
        //Debug.Log("Target Pos: " +  target + ": " + target.position);
        //Debug.Log("Poop Pos: " + transform.position);
        Throw();
    ...
    void Throw()
    {
        Vector3 targetPos = (target - spawnPos.position).normalized;
        //Debug.Log("Target Dir: " + targetPos);
        rb.AddForce(targetPos * throwPower, ForceMode.Force);
    }
